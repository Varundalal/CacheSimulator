...
{
    openTraceAct = new QPushButton("Generate Trace File");
    openTraceAct->setStatusTip(tr("Read Trace File"));
    connect(openTraceAct, SIGNAL(clicked()),this, SLOT(readTrace()));
...
void MainWindow::createDockWindows()
{
    ...
    QDockWidget *dock2 = new QDockWidget(tr("Shapes"),this);
    dock2->setAllowedAreas(Qt::RightDockWidgetArea);
    window2= new QWidget(dock2);
    ...
    grid->addWidget(openTraceAct,14,1);
    grid->addWidget(quit,18,3);

    obj1 = new GLobj(this);
    obj1->setFixedSize(400,400);


    QGridLayout *grid2 = new QGridLayout;
    grid2->addWidget(obj1,0,0);


    window1->setLayout(grid);
    window2->setLayout(grid2);

    dock->setWidget(window1);
    dock2->setWidget(window2);
